...
void CPU::_ld_nn_SP() {
    ...
    uint16_t addr = (msb_n << 8) | lsb_n;
    this->_mem_write_byte(addr, this->_SP.raw);
...
void CPU::_push_rr() {
    ...
    this->_SP.raw -= 1;
    this->_mem_write_byte(this->_SP.raw--, reg->bytes[1]);
    this->_mem_write_byte(this->_SP.raw, reg->bytes[0]);
...
void CPU::_pop_rr() {
    ...
    uint8_t lsb = this->_mem_read_byte(this->_SP.raw++);
    uint8_t msb = this->_mem_read_byte(this->_SP.raw++);
    reg->raw = (msb << 8) | lsb;
...
void CPU::_add_A_HL() {
    this->_read_and_increment_PC();
    uint8_t data = this->_mem_read_byte(this->_HL.raw);
...
void CPU::_inc_HL() {
    ...
    this->_mem_write_byte(this->_HL.raw, data + 1);
    this->_cycles += MACHINE_CYCLE*3;
}
